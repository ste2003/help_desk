...
module.exports = {
	...
	buscarNombre: function(idDocEntidad,callback){
		var nom = idDocEntidad;
		console.log(nom);
		dbConn.conectar(function(db){
			db.collection(coleccionSelecc)
			.findOne({ "nombre" : nom }, function(err,resultado) {
    			if(err) throw err;
    			callback(resultado);
  			});   
		});
	...
	buscarTodos: function(callback){
		...
		dbConn.conectar(function(db){
			db.collection(coleccionSelecc).
			find({}).sort({'nombre': 1}).toArray(function(err, resultado) {
    			if(err) {console.log('error en clienteDAO.js');
    				throw err;}
    			else
    			{console.log('exito en clienteDAO.js');
